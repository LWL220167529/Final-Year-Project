...
const PlanGeneration = () => {
  ...
  const [hotData, setHotData] = useState(SelectedHotel);
...
console.log(CityString); 
    ...
    ]).then(([cityData, mainData, resData]) => {
...
{isLoading ? (
  ...
  hotData ? (
    <EhotelContent
    title={hotData?.Hotel}
    rating={hotData?.rating}
    category={hotData?.category}
    imageSrc={hotData?.ImageSrc || 'https://cdn2.iconfinder.com/data/icons/building-vol-2/512/restaurant-512.png'}
  />
  ) : (
    <Text>No data available</Text>
  )
...
<View style={{position:'absolute', backgroundColor: '#FAE7F3', top: 20, right:0}}>
          <MaterialCommunityIcon name='fountain-pen' size={30} color='gold' />
          <Text>Itinerary</Text></View>
{showPlanBtn && (
